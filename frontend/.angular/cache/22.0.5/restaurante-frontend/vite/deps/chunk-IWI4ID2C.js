...
function createRootTView(rootSelectorOrNode, componentDef, componentBindings, directives) {
  const tAttributes = rootSelectorOrNode ? ["ng-version", "22.0.5"] : extractAttrsAndClassesFromSelector(componentDef.selectors[0]);
...
function getBaseDocUrl() {
  const full = VERSION.full;
  const isPreRelease = full.includes("-next") || full.includes("-rc") || full === "22.0.5";
  const prefix = isPreRelease ? "next" : `v${VERSION.major}`;
  return `https://${prefix}.angular.dev`;
}
...
//# sourceMappingURL=chunk-IWI4ID2C.js.map
